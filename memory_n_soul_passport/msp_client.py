...
class MSPClient:
    ...
    def query_semantic_patterns(self, tags: List[str], limit: int = 5, **kwargs) -> List[Dict]:
        """Query semantic patterns (Intuition Stream) - Mock for now"""
        return self.query_by_tags(tags, limit=limit)

    def query_reflections(self, tags: List[str], limit: int = 5, **kwargs) -> List[Dict]:
        """Query reflections (Reflection Stream) - Mock for now"""
        return []

    def query_recent_episodes(self, limit: int = 5, **kwargs) -> List[Dict]:
        """Alias for get_recent_episodes"""
        return self.get_recent_episodes(limit=limit)

    def query_narrative_chain(self, context_id: str, limit: int = 5, **kwargs) -> List[Dict]:
        """Alias/Mock for Narrative Stream"""
        return self.get_recent_episodes(limit=limit)

    def query_by_salience(self, min_score: float = 0.5, limit: int = 5, **kwargs) -> List[Dict]:
        """Alias for Salience Stream"""
        return self.query_by_ri(min_ri=min_score, max_results=limit)

    def query_sensory_memories(self, tags: List[str], limit: int = 5, **kwargs) -> List[Dict]:
        """Alias for Sensory Stream"""
        return self.query_by_qualia(min_intensity=0.5, max_results=limit)
