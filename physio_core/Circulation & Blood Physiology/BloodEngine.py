...
class BloodEngine:
    ...
    No scheduler, no user interaction, no reflex.
    """
    ...
    # ========================================================
    # Core Update (called by BG runner)
    # ========================================================
    def update(self, flow_factor: float = 1.0):
        """
        flow_factor: external modifier (e.g. from hemodynamics)
        """
        ...
        # ---- passive decay (lazy but bounded by BG loop) ----
        for h_id in list(self.plasma.keys()):
            self._apply_decay(h_id, now, eff_flow)
